...
export default function ExpensesPage() {
  ...
  const EXPENSE_CATEGORIES = ['Жетоны для мойки', 'Такси', 'Заказы/Покупки', 'Ремонт', 'Смог чек', 'Выезд из парковки LAX', 'Заправка']
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (total > 5) { alert('Максимум 5 файлов'); return }
  ...
  async function markPaid(paid: boolean) {
    ...
    const supabase = createClient()
    const ids = [...selected]
    await supabase.from('expenses').update({ paid }).in('id', ids)
    setSelected(new Set())
    load()
  }
  ...
  const ilyaCarIds = new Set(cars.filter(c => c.owner_group === 'ilya').map(c => c.id))
  ...
  const filtered = expenses.filter(e => {
    ...
    const matchGroup = ownerTab === 'ilya'
      ? ilyaCarIds.has(e.car_id)
      : !ilyaCarIds.has(e.car_id)
    return matchMonth && matchCar && matchGroup
  })
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          {(['main', 'ilya'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => { setOwnerTab(tab); setFilterCar('') }}
      ...
      <div className="flex items-center gap-4 mb-6">
        <Select
          ...
          options={[{ value: '', label: 'Все авто' }, ...tabCars.map(c => ({ value: c.id, label: `${c.make} ${c.model} ${c.plate}` }))]}
        />
        {filtered.length > 0 && (
          <span className="text-sm text-gray-500">Итого: <strong className="text-gray-900">${totalFiltered.toFixed(2)}</strong></span>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr className="text-left text-gray-500">
                ...
                <th className="px-6 py-4 font-medium">Авто</th>
                <th className="px-6 py-4 font-medium">Статья</th>
                <th className="px-6 py-4 font-medium text-right">Сумма</th>
                <th className="px-6 py-4 font-medium text-center">Оплата</th>
                <th className="px-6 py-4 font-medium text-center">Чеки</th>
            ...
            <tbody>
                ...
                <React.Fragment key={exp.id}>
                  ...
                  <tr className={`border-b last:border-0 hover:bg-gray-50 ${selected.has(exp.id) ? 'bg-indigo-50/40' : exp.paid ? 'bg-emerald-50/20' : ''}`}>
                    ...
                    <td className="px-6 py-4 font-medium">
                      {exp.title}
                      {exp.notes && <span className="text-gray-400 font-normal ml-1.5 text-xs">· {exp.notes}</span>}
                    </td>
                    <td className="px-6 py-4 text-right font-semibold">${Number(exp.amount).toFixed(2)}</td>
                    <td className="px-6 py-4 text-center">
                      {exp.paid ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-600">Оплачено</span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-medium bg-rose-50 text-rose-400">Не оплачено</span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-center">
                      {exp.files?.length > 0 ? (
                        <div className="flex items-center justify-center gap-1">
                          {exp.files.map((f, i) => (
                            <div key={i} className="flex gap-0.5">
                              <button onClick={() => handlePreview(f)} onMouseDown={e => e.preventDefault()} className="p-1 rounded hover:bg-blue-50 text-blue-500 transition-colors" title="Открыть"><ImageIcon size={14} /></button>
                              <button onClick={() => handleDownload(f)} className="p-1 rounded hover:bg-blue-50 text-blue-500 transition-colors" title="Скачать"><Download size={14} /></button>
                            </div>
                          ))}
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={8} className="px-6 py-8 text-center text-gray-400">Расходов нет</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {!loading && filtered.length > 0 && (
        <div className="flex items-center gap-8 mt-4 px-1">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-500">Осталось выплатить:</span>
      ...
      {deleteExpense && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setDeleteExpense(null)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-gray-900 mb-2">Удалить расход?</h2>
            <p className="text-sm text-gray-500 mb-6">
              <span className="font-medium text-gray-800">{deleteExpense.title}</span> · ${Number(deleteExpense.amount).toFixed(2)} будет удалён безвозвратно.
            </p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-semibold mb-5">{editExpense ? 'Редактировать расход' : 'Добавить расход'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Статья расходов</label>
              ...
              <div>
                ...
                <input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Доп. информация..." className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                ...
                <Select
                  ...
                  options={[
                    { value: '', label: 'Выберите авто' },
                    { value: '__main__', label: '— Основной парк —', disabled: true },
                    ...cars.filter(c => (c.owner_group || 'main') === 'main').map(c => ({ value: c.id, label: `${c.make} ${c.model} ${c.plate}` })),
                    { value: '__ilya__', label: '— Парк Ильи —', disabled: true },
                    ...cars.filter(c => c.owner_group === 'ilya').map(c => ({ value: c.id, label: `${c.make} ${c.model} ${c.plate}` })),
                  ]}
                />
              </div>
              ...
              {/* File upload */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Фото чеков (до 5 штук)</label>
                {/* Existing files */}
                ...
                {(files.length + existingFiles.length) < 5 && (
                  ...
                  >
                    <Paperclip size={15} /> Прикрепить фото
                  </button>
                )}
