            ...
            <div key={c.id} className="bg-white rounded-[18px] p-5 card-shadow border border-gray-100/80">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  {c.status === 'waiting' && (
                    <button
                      onClick={() => setStatus(c, 'in_repair')}
                      className="text-xs bg-blue-50 hover:bg-blue-100 text-blue-600 font-medium px-3 py-1.5 rounded-lg border border-blue-100 transition-colors whitespace-nowrap"
                    >
                      → В ремонт
                    </button>
                  )}
                  {c.status === 'in_repair' && (
                    <button
                      ...
                      className="text-xs bg-green-50 hover:bg-green-100 text-green-600 font-medium px-3 py-1.5 rounded-lg border border-green-100 transition-colors whitespace-nowrap"
                    >
                      → Завершить
                    </button>
                  )}
                  ...
                  <button onClick={() => handleDelete(c)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors">
                    <X size={15} />
                  </button>
                </div>
              </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {cars.filter(c => !c.delisted).map(c => (
              ...
              <div>
                ...
                <select
                  value={form.status}
                  onChange={e => setForm(f => ({ ...f, status: e.target.value as Claim['status'] }))}
                  className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    type="checkbox"
                    checked={form.initial_payment_received}
                    onChange={e => setForm(f => ({ ...f, initial_payment_received: e.target.checked }))}
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    type="checkbox"
                    checked={form.supplement_paid}
                    onChange={e => setForm(f => ({ ...f, supplement_paid: e.target.checked }))}
                    className="w-4 h-4 rounded accent-blue-600"
                  />
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    type="checkbox"
                    checked={form.still_listed}
                    onChange={e => setForm(f => ({ ...f, still_listed: e.target.checked }))}
                    className="w-4 h-4 rounded accent-blue-600"
                  />
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Описание повреждений, детали клейма..."
                  rows={3}
